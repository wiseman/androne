...
/**
 * This is a utility to detect bluetooth headset connection and
 * establish audio connection for android API >= 8. This includes a
 * work around for API < 11 to detect already connected headset before
 * the application starts. This work around would only fails if Sco
 * audio connection is accepted but the connected device is not a
 * headset.
 *
 * @author Hoan Nguyen
 *
...
public abstract class BluetoothHeadsetUtils
{
     ...
     * startBluetoothSco().  This is a work around for API < 11 to
     * detect if a headset is connected before the application starts.
     *
     * The official documentation for startBluetoothSco() states
     *
     * "This method can be used by applications wanting to send and
     *  received audio to/from a bluetooth SCO headset while the phone
     *  is not in call."
     *
     * Does this mean that startBluetoothSco() would fail if the
     * connected bluetooth device is not a headset?
     *
     * Thus if a call to startBluetoothSco() is successful, i.e
     * mBroadcastReceiver will receive an
     * ACTION_SCO_AUDIO_STATE_CHANGED with intent extra
     * SCO_AUDIO_STATE_CONNECTED, then we assume that a headset is
     * connected.
     *
     * @return false if device does not support bluetooth or current
     *         platform does not supports use of SCO for off
     *         call.
     */
    ...
    private boolean startBluetooth()
    {
        ...
        if (mBluetoothAdapter != null)
        {
            ...
            {
                Log.d(TAG, "Bluetooth sco is available off-call");
                ...
                mIsCountDownOn = true;
                // mCountDown repeatedly tries to start bluetooth Sco
                // audio connection.
                mCountDown.start();
        ...
        Log.d(TAG, "Bluetooth sco is NOT available off-call");
        return false;
    }
    ...
    private boolean startBluetooth11()
    {
        ...
        Log.d(TAG, "Bluetooth sco is NOT available off-call/couldn't get proxy");
        return false;
    }
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            {
                Log.d(TAG, "Got ACTION_ACL_CONNECTED");
                mConnectedHeadset = intent.getParcelableExtra(
                    BluetoothDevice.EXTRA_DEVICE);
                BluetoothClass bluetoothClass = mConnectedHeadset.getBluetoothClass();
                if (bluetoothClass != null)
                {
                    // Check if device is a headset. Besides the 2
                    // below, are there other device classes also
                    // qualified as headset?
            ...
            {
                Log.d(TAG, "Got ACTION_ACL_DISCONNECTED");
            ...
            {
                Log.d(TAG, "Got ACTION_SCO_AUDIO_STATE_CHANGED");
                ...
                {
                    Log.d(TAG, "state is SCO_AUDIO_STATE_CONNECTED");
                    mIsOnHeadsetSco = true;

                    if (mIsStarting)
                    {
                        Log.d(TAG, "we are starting");
                        // When the device is connected before the
                        // application starts, ACTION_ACL_CONNECTED
                        // will not be received, so call
                    ...
                    if (mIsCountDownOn)
                    {
                        Log.d(TAG, "canceling countdown");
                        mIsCountDownOn = false;
                        mCountDown.cancel();
                    }

                    // override this if you want to do other thing
                    // when Sco audio is connected.
                    onScoAudioConnected();
                ...
                else if (state == AudioManager.SCO_AUDIO_STATE_DISCONNECTED)
                {
                    ...
                    // call to startBluetooth() which at that stage we
                    // do not want to do anything. Thus the if
                    // condition.
                    if (!mIsStarting)
                    {
                        ...
                        // startBluetoothSco() will not be successful.
                        mAudioManager.stopBluetoothSco();

                        // override this if you want to do other thing
                        // when Sco audio is disconnected.
                        onScoAudioDisconnected();
                    }
                }
            }
            else {
                Log.d(TAG, "Got some other action");
            }
        }
    };
    ...
    private BluetoothProfile.ServiceListener mHeadsetProfileListener = new BluetoothProfile.ServiceListener()
    {
        ...
        public void onServiceConnected(int profile, BluetoothProfile proxy)
        {
            ...
            // on and off the headset.  So register for broadcast of
            // connection states.
            mContext.registerReceiver(
            ...
            // immediate audio connection.  So register for broadcast
            // of audio connection states. This broadcast will only be
            // sent if startVoiceRecognition returns true.
            mContext.registerReceiver(
                mHeadsetBroadcastReceiver,
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            else // audio
            {
                ...
                if (state == BluetoothHeadset.STATE_AUDIO_CONNECTED)
                {
                    ...
                    // override this if you want to do other thing
                    // when headset audio is connected.
                    onScoAudioConnected();
                }
                else if (state == BluetoothHeadset.STATE_AUDIO_DISCONNECTED)
                {
                    ...
                    // override this if you want to do other thing
                    // when headset audio is disconnected.
                    onScoAudioDisconnected();
    ...
    private CountDownTimer mCountDown11 = new CountDownTimer(10000, 1000)
    {
        ...
        public void onTick(long millisUntilFinished)
        {
            ...
            // stick always returns true if the countDownInterval is
            // set to 1000.  It is somewhere in between 500 to a 1000.
        ...
        public void onFinish()
        {
            ...
            // successful.  Should implement something to inform user
            // of this failure
            mIsCountDownOn = false;
            Log.d(TAG, "\nonFinish fail to connect to headset audio"); //$NON-NLS-1$
        }
